...
**Status:** Prerequisites done, flocking integration NOT started (deferred — not
...
the flocking-side work — Steps 3–5: make flocking consume the package, rewrite
`look()`, add the `push-to-flocking` mirror rewrite, verify.

## Why
`world.look(cell, radius)` in `src/simulation.ts` filters **every** cell against the
...
Not published to npm yet — flocking will consume it locally via the workspace
...
## Decision (locked)
Quadtree is a **standalone library**, not part of flocking. It lives in
`mono/npm/quadtree` as `@amatiasq/quadtree` (publishing to npm is optional/later
— local consumption below needs no publish), and flocking consumes it **by package
name** via Bun workspaces. No copy/build step, no vendoring into flocking. The root
...
- **Each consumer declares the libs as members**: `flocking/package.json` gets
...
The mirror still works because flocking imports `@amatiasq/quadtree` by name either
way — only `package.json` differs between the mono checkout and the standalone
repo, and `push-to-flocking` rewrites it (Step 3).
...
## Step 3 — flocking consumes it + the mirror rewrite
In `flocking/package.json`:
```jsonc
{
  "workspaces": ["../npm/*"],
...
in dev with live edits. Vite bundles it into flocking's app build normally.

The standalone `amatiasq/flocking` repo has no `../npm` and isn't a workspace, so
`push-to-flocking` must transform the mirrored `package.json`:
...
```jsonc
// mono checkout                         // mirrored amatiasq/flocking
...
In `simulation.ts`, per `step()` (positions move every tick, so rebuild each frame):
...
## Step 5 — verify
- All flocking specs green (behaviour must be identical for small N).
